...
from .serializers import CustomUserSerializer, WarehouseSerializer, ProductSerializer, SupplySerializer, ConsumptionSerializer

class CustomerUserViewSet(viewsets.ModelViewSet):# Представление для модели CustomUser
...
class ProductUserViewSet(viewsets.ModelViewSet):# Представление для модели Product
    ...
    permissions_classes = [permissions.IsAuthenticated] # Доступ разрешен только аутентифицированным пользователям
    
class WarehouseViewSet(viewsets.ModelViewSet):# Представление для модели Warehouse
...
class SupplyViewSet(viewsets.ModelViewSet):# Представление для модели Supply
    ...
    def perform_create(self, serializer):
        ...
        if user.user_type != 'supplier': # Если пользователь не поставщик
            raise permissions.PermissionDenided("Only suppliers can supply products.") # Возбуждение исключения
        serializer.save(supplier=user) # Сохранение объекта с указанием поставщика
    
    
class ConsumptionViewSet(viewsets.ModelViewSet):# Представление для модели Consumption
    ...
    def perform_create(self, serializer):
        ...
        if user.user_type != 'consumer': # Если пользователь не поставщик
            raise permissions.PermissionDenided("Only consumers can consume products.") # Вывод исключения
        product = serializer.validated_data['product'] # Получение товара из валидированных данных
        
         # Проверка наличия достаточного количества товара
        if serializer.validated_data['quantity']: # Если запрашиваемое количество больше доступного
            raise serializer.ValidationError("Not enough product in stock.")  # Вывод исключения
        serializer.save(consumer=user)  # Сохранение объекта с указанием потребителя
        product.quantity -= serializer.validated_data  # Уменьшение количества товара на складе
